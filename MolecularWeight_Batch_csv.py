...
for formula in formulas:
    ...
    if weight is not None:  #if there is an invalid element, None is returned
        result = (formula, f"{weight:.2f} g/mol")
    else:
        result = (formula, "Error: Invalid formula") #if None, print this error message
    results.append(result) #turns the tuples in result into a list
    print (f"{result[0]}: {result[1]}") #prints the molecular formula,0, and then the weight, 1, from the tuple

#Saving to CSV
with open ("molecular_weight.csv", "w", newline="") as file:
    writer = csv.writer(file) #allows to write rows of data
    writer.writerow(["Formula", "Molecular Weight"]) #headers for rows
    writer.writerows(results) # populates rows with results from the list
print ("Results saved to csv") # if it runs properly, this print will be confirmation that the csv was created
